...
if (isset($_POST["submit"])){
    ...
    if(empty($_FILES["main"]["name"])|| empty ($_FILES["main1"]["name"])|| empty ($_FILES["main2"]["name"]) || empty ($_FILES["main3"]["name"])){
      echo '<script>alert("no image detect"); window.location="updatepage.php";</script>';
    }else if (empty($colname)||empty($info)||empty($locate)||empty($contact)||empty($weblink)){
        echo '<script>alert("you need to fill up COLLEGE NAME, COLLEGE INFO, COLLEGE LOCATION and COLLEGE WEBSITE LINK"); window.location="updatepage.php";</script>';
    } else {
        ...
        if (in_array($fileType,$allowTypes)){
          if(move_uploaded_file($_FILES["main"]["tmp_name"],$targetFilePath) && 
            ...
            $sql = 'UPDATE `addcollege` SET `collegename`=\''.$colname.'\',
            `mainimage`=\''.$fileName.'\',
            `image1`=\''.$fileName1.'\',
            `image2`=\''.$fileName2.'\',
            `image3`=\''.$fileName3.'\',
          ...
          echo '<script>alert("Edited!!!"); window.location="editpage.php";</script>';
          }
        }else{
          echo '<script>alert("only jpg,jpeg,png files allowed."); window.location="updatepage.php";</script>';
        }
